...
| Page token | `META_PAGE_ACCESS_TOKEN` is still a placeholder. Page identity, granted permissions, token expiry, and Page-level subscription cannot yet be checked. |
| Supabase platform access | At 00:28 EAT on 16 September, the official status API reported a partially degraded service with the Management API under scheduled maintenance. The CLI returned `Service temporarily unavailable for scheduled maintenance`, and the connected Supabase tools returned internal errors when listing projects. The configured project's REST gateway, Auth, and Storage endpoints all returned HTTP 200, so the project runtime remained reachable while dashboard permission retrieval failed. |
...
| Cloudflare bindings | The live Worker lacks the exact required Meta/Supabase/admin binding names. Existing names include `Meta_app_Id`, `Supabase_Service_role`, and `Supabase_publishable_key`; these do not match the code. Its environment is also `development`, despite serving the production domain. |
...
Generate/authorize the user token with these permissions, then select **Get Page Access Token** and the MobDeals Page. Ensure the token comes from this app and this Page. For ongoing operation, obtain a long-lived Page token through Meta's supported token flow; a temporary Explorer token is only suitable for initial checks.
...
3. **Cloudflare bindings:** install the exact case-sensitive keys listed below using `.dev.vars`, after the valid Page token is supplied. Use the production configuration rather than copying the local `ENVIRONMENT=development` setting. Review old incorrectly named bindings before removing them.
...
Meta app identity, API version, subscription state, Cloudflare binding names, public URLs, Supabase data-plane responses, the Supabase status API, and the Supabase CLI management error were checked directly. Meta's detailed Pages/permissions documentation returned rate-limit or fetch errors, so confirm the suggested permission set and access requirements in the authenticated app dashboard before submission. Reference destinations: [Pages publishing](https://developers.facebook.com/docs/pages-api/posts/), [Page webhooks](https://developers.facebook.com/docs/graph-api/webhooks/getting-started/webhooks-for-pages/), and [permissions](https://developers.facebook.com/docs/permissions/).

Only the local API version and this checklist were changed. No remote configuration, database data, subscriptions, deployment, or outbound switches were changed.
